...
void	draw_wall(t_data *data, t_draw *draw, t_texture *texture)
{
	...
	float			step;//贴图和墙的比例ratio
	float			tex_pos;
	int				y;
	unsigned char	*pixel;//像素指针
		...
		t_ray_casting *rc)
{
	t_draw		draw;
	t_texture	*texture;
	float		tex_x;//texture在x方向的坐标
	int			texture_index;//用哪张texture

	tex_x = rc->tex_x;//[0,1)之间的小数
	...
	if (!texture || !texture->addr || !texture->img)//如贴图加载失败，贴纯色墙
	{
		draw_simple_wall(data, x, draw.wall_start, draw.wall_end);
		return ;
	}
	draw.tex_x_int = (int)(tex_x * (texture->width - 1)); //将小数转换为int，实际贴图x坐标
